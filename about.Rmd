...
# trying a new way to get census data to enable geom_sf
...
income_final <- income1 %>% 
  full_join(zip_county_cross, by = c("Id2" = "COUNTY")) %>% 
  select(-Id2)

# join income data with office data

incomebyoffice <- s_offices %>% 
  left_join(income_final, by = c("zip_code" = "ZIP")) %>% 
  rename(county = Geography, median_income = "Median income (dollars); Estimate; Households") %>% 
...
map_income <- get_acs(geography = "county",
